...
class KotlinInput(private val text: String, file: KtFile) : Input() {
     ...
     */
    @Throws(FormatterException::class)
    internal fun characterRangeToTokenRange(
        offset: Int,
        length: Int,
    ): Range<Int> {
    ...
    private fun makePositionToColumnMap(toks: List<KotlinTok>) = ImmutableMap.copyOf(toks.map { it.position to it.column }.toMap())
    ...
    override fun getLineNumber(inputPosition: Int) = StringUtil.offsetToLineColumn(text, inputPosition).line + 1

    override fun getColumnNumber(inputPosition: Int) = StringUtil.offsetToLineColumn(text, inputPosition).column
}
